...
app.post('/saveAnswers', [
    ...
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'answers.json'), existingAnswers.map(JSON.stringify).join('\n') + '\n');
...
app.post('/saveFortuneAnswers', async (req, res) => {
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'fortune_answers.json'), existingAnswers.map(JSON.stringify).join('\n') + '\n');
...
app.post('/saveMBTIAnswers', async (req, res) => {
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'mbti_answers.json'), existingAnswers.map(JSON.stringify).join('\n') + '\n');
...
app.delete('/deleteAnswer/:index', requireLogin, async (req, res) => {
    ...
    try {
        ...
        if (index >= 0 && index < answers.length) {
            ...
            await fs.writeFile(path.join(__dirname, 'answers.json'), answers.map(JSON.stringify).join('\n') + '\n');
...
// 서버 시작 시 필요한 JSON 파일들이 존재하는지 확인하고, 없으면 생성하는 함수
async function initializeJSONFiles() {
    const files = ['answers.json', 'design.json', 'home_design.json', 'fortune_answers.json', 'mbti_answers.json'];
    for (const file of files) {
        try {
            await fs.access(path.join(__dirname, file));
        } catch (error) {
            if (error.code === 'ENOENT') {
                console.log(`${file} not found. Creating an empty file.`);
                await fs.writeFile(path.join(__dirname, file), '[]', 'utf-8');
            } else {
