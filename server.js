...
const readAnswers = async () => {
    try {
        const data = await fs.readFile(path.join(__dirname, 'uploads', 'answers.json'), 'utf-8');
...
app.post('/saveDesign', requireLogin, upload.fields([
    ...
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'uploads', 'design.json'), JSON.stringify(designData), 'utf-8');
        res.json({ success: true });
    } catch (error) {
...
app.post('/saveHomeDesign', requireLogin, upload.fields([
    ...
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'uploads', 'home_design.json'), JSON.stringify(homeDesignData), 'utf-8');
        res.json({ success: true });
    } catch (error) {
...
app.post('/saveAnswers', [
    ...
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'uploads', 'answers.json'), existingAnswers.map(JSON.stringify).join('\n') + '\n');
...
app.post('/saveFortuneAnswers', async (req, res) => {
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'uploads', 'fortune_answers.json'), existingAnswers.map(JSON.stringify).join('\n') + '\n');
...
app.post('/saveMBTIAnswers', async (req, res) => {
    try {
        ...
        await fs.writeFile(path.join(__dirname, 'uploads', 'mbti_answers.json'), existingAnswers.map(JSON.stringify).join('\n') + '\n');
...
app.get('/api/home_design', async (req, res) => {
    try {
        const data = await fs.readFile(path.join(__dirname, 'uploads', 'home_design.json'), 'utf-8');
        res.json(JSON.parse(data));
    } catch (error) {
        if (error.code === 'ENOENT') {
            res.status(404).json({ error: 'Home design data not found' });
        } else {
            res.status(500).json({ error: 'Failed to load home design data' });
        }
    }
});

app.get('/api/fortune_answers', async (req, res) => {
    try {
        const data = await fs.readFile(path.join(__dirname, 'uploads', 'fortune_answers.json'), 'utf-8');
        res.json(JSON.parse(data));
    } catch (error) {
        if (error.code === 'ENOENT') {
            res.status(404).json({ error: 'Fortune answers data not found' });
        } else {
            res.status(500).json({ error: 'Failed to load fortune answers data' });
        }
    }
});

app.get('/api/mbti_answers', async (req, res) => {
    try {
        const data = await fs.readFile(path.join(__dirname, 'uploads', 'mbti_answers.json'), 'utf-8');
        res.json(JSON.parse(data));
    } catch (error) {
        if (error.code === 'ENOENT') {
            res.status(404).json({ error: 'MBTI answers data not found' });
        } else {
            res.status(500).json({ error: 'Failed to load MBTI answers data' });
        }
    }
});
...
app.delete('/deleteAnswer/:index', requireLogin, async (req, res) => {
    ...
    try {
        ...
        if (index >= 0 && index < answers.length) {
            ...
            await fs.writeFile(path.join(__dirname, 'uploads', 'answers.json'), answers.map(JSON.stringify).join('\n') + '\n');
